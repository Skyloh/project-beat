...
namespace System.Collections.Generic
{
    ...
    {
        private const string ERR_QUEUE_EMPTY = "Queue empty!";

        private const uint ARRAY_MAX_LENGTH = 0x7FFFFFC7;
        ...
        // copied from EnumeratorHelpers.cs
        private static IEnumerator<T> GetEmptyEnumerator<T>() => ((IEnumerable<T>)Array.Empty<T>()).GetEnumerator();

        // copied from EnumeratorHelpers.cs
        private T[] ToArray<T>(IEnumerable<T> source, out int length)
        {
                ...
                if (en.MoveNext())
                {
                    ...
                    while (en.MoveNext())
                    {
                        if (count == arr.Length)
                        {
                            ...
                            {
                                newLength = ARRAY_MAX_LENGTH <= count ? count + 1 : int.MaxValue; // Nathan's note: see other note for why im confusion
                            }
        ...
        private void Grow(int minCapacity)
        {
            ...
            if ((uint)newcapacity > ARRAY_MAX_LENGTH) newcapacity = int.MaxValue; // Nathan's note: Array.MaxValue was here, but that doesn't make sense to me since newcap is an int.
